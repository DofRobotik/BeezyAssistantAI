...
class Agent:
    ...
    def __init__(self, 
        ...
        self.station_prompt_list = "\n".join([f"- {s['name']}: {s['property']}" for s in self.stations])
...
"""
    def classify_intent(self, text: str) -> Dict[str, Any]:
        ...
        if not data:
            print(f"[AGENT-ERROR] LLM'den geçerli JSON alınamadı. Yanıt: {raw_response}")
        ...
        if data.get("request_type") not in ["navigation", "iot", "chat"]:
            print(f"[AGENT-WARN] LLM'den bilinmeyen request_type alındı: {data.get('request_type')}")
            return {"request_type": "chat", "reason": "Bilinmeyen request_type"}
            
        print(f"[AGENT-CLASSIFY] Karar: {json.dumps(data)}")
        return data

    def execute_iot_command(self, target_code: str, action: str, value: Any, lang: Literal["en", "tr"]) -> Tuple[bool, str]:
        """
        (BLOKE EDİCİ) LLM'den gelen IoT komutunu çalıştırır.
        'iot_executor' içinde çalıştırılmalıdır.
        """
        print(f"[AGENT-IOT] Komut yürütülüyor: Cihaz={target_code}, Eylem={action}, Değer={value}")
        ...
        try:
            ...
            if target_code in self.light_device_map:
                ...
                if action == "turn_on":
                    ...
                    self.iot.send_data_for_light_func(group, index, switch=True, dimming=150) # (Varsayılan parlaklık)
                    return True, f"{target_code} turned on." if lang == "en" else f"{target_code} açıldı."
                elif action == "turn_off":
                    # iot.py'den 'send_data_for_light_func' çağrısı
                    self.iot.send_data_for_light_func(group, index, switch=False, dimming=0)
                    return True, f"{target_code} turned off." if lang == "en" else f"{target_code} kapatıldı."
                else:
                    return False, f"'{action}' is an invalid action for lights." if lang == "en" else f"Işıklar için '{action}' geçersiz bir eylemdir."
            ...
            #     return True, msg_en if lang == "en" else msg_tr
                ...
                return False, f"Device with code '{target_code}' not found." if lang == "en" else f"'{target_code}' kodlu cihaz sistemde bulunamadı."
    ...
    def is_yes(text:str) -> bool:
        t = text.strip().lower()
        return any(x in t for x in ["yes","yeah","yep","please do","ok","sure","confirm","go ahead","start","guide me","let's go","let us go","affirmative","evet","aynen","doğru"])

    @staticmethod
    def is_no(text:str) -> bool:
        t = text.strip().lower()
        return any(x in t for x in ["no","nope","not now","cancel","stop","don't","do not","negative","hayır","yok","istemiyorum"])