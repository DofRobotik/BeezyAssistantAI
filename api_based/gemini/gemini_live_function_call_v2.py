...
class GeminiAssistant:
    ...
    async def send_realtime(self):
        while True:
            ...
            try:
                blob = types.Blob(
                    data=msg["data"],
                    # Eğer sen CHUNK’ı 16kHz gönderiyorsan; değilse rate’i kendi ayarına göre düzelt.
    ...
    async def control_mic(self):
        ...
        while True:
            try:
                # 1) Kullanıcı konuşmaya başlamak için Enter'a basar
                ...
                # MANUAL VAD: Kullanıcı aktivitesi başlıyor
                try:
                ...
                # 2) Kullanıcı konuşmayı bitirmek için tekrar Enter'a basar
                await asyncio.to_thread(input)

                print("⚪ Kayıt durdu. İşleniyor...")
                self.is_recording = False
                # Mic stream'in kapanmasına çok küçük bir süre ver
                ...
                # Güvenlik için aktif bir activity varsa kapatmayı dene
                self.is_recording = False
                try:
                    await self.session.send_realtime_input(
                        activity_end=types.ActivityEnd()
    ...
    async def play_audio(self):
        ...
        while True:
            ...
            # Kayıt sırasında model sesini çalma (opsiyonel ama önerilir)
            if MUTED_WHILE_RECORDING and self._playback_muted:
                continue
